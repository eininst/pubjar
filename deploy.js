...
module.exports = function(config) {
    ...
    conn.on('ready', function() {
        ...
        conn.sh(`find ${config.pubPath} -maxdepth 1 -name pubjar -type d -print`)
            ...
            jar = result.replace('\n', '');
            return conn.sh(`ps -ef|grep ${jar} |grep -v 'grep' |awk '{print $2}'`);
        }).then(function(result) {
            if (result) {
                var ids = result.replace(/\n/g, ' ');
                return conn.sh(`sudo kill -9 ${ids}`);
            } else {
                return;
            }
        }).then(function(result) {
            return conn.sh(`$JAVA_HOME/bin/java -javaagent:/root/publish/quasar-core-0.7.5.jar  -jar ${jar} 1>${deployPath}/stdout.log 2>${deployPath}/stderr.log&`);
        }).then(function(result) {
